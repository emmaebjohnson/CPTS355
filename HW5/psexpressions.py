...
class Expr:
    ...
    def eval(self, ps_env):
        # if self.value[0] == '/':
        #     py_string = []
        #     pystring = pystring + self.value[1:]
        #     ps_env.opstack.opPush(pystring)
        """
...
class PSName(Expr):
    ...
    def eval(self,ps_env):
        ...
        #      ps_env.get(self.var_name)
        
        else:
             variable = ps_env.lookup(self.var_name)
             variable.apply(ps_env, self.var_name) if isinstance(variable, CodeArrayValue) else ps_env.opPush(variable)
...
class CodeArrayValue(Value):
    ...
    def apply(self, ps_env, name):
        """`ps_env` is the PSOperators object that include the `opstack` and `dictstack`. """
        """ TO-DO in part2 """

        if ps_env.scope == "static":
            ps_env.dictPush(ps_env.static_link(name), {})
